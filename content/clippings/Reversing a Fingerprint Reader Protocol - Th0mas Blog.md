---
title: "Reversing a Fingerprint Reader Protocol - Th0mas Blog"
source: "https://blog.th0m.as/misc/fingerprint-reversing/"
author:
...
![](https://blog.th0m.as/misc/fingerprint-reversing/fingerprint_scan.gif)

Scan of not-my-finger.

I recently got a new Laptop (`Lenovo Ideapad 5 - 15are05`). Linux compatibility is quite good, as documented in the [Arch Wiki](https://wiki.archlinux.org/index.php/Lenovo_IdeaPad_5_15are05). One part that is not working is the fingerprint reader. In this blog post I describe my approach to reverse engineering the fingerprint reader's windows driver and USB protocol. This results in a python script, capable of streaming images from the sensor at around 15 FPS. Fingerprint recognition, as implemented by for example [libfprint](https://fprint.freedesktop.org/), is not available yet. More recently, a [Discord community](https://gitlab.freedesktop.org/libfprint/libfprint/-/issues/376#note_906650) was formed, with the goal to support all kinds of Goodix devices in libfprint.
...
- [Contents](https://blog.th0m.as/misc/fingerprint-reversing/#contents)
- [Getting an overview of the device.](https://blog.th0m.as/misc/fingerprint-reversing/#getting-an-overview-of-the-device)
	- [Logging USB traffic](https://blog.th0m.as/misc/fingerprint-reversing/#logging-usb-traffic)
	- [Windows Driver](https://blog.th0m.as/misc/fingerprint-reversing/#windows-driver)
		- [Finding the driver](https://blog.th0m.as/misc/fingerprint-reversing/#finding-the-driver)
		- [Another way of finding all relevant driver files.](https://blog.th0m.as/misc/fingerprint-reversing/#another-way-of-finding-all-relevant-driver-files)
		- [Usermode Driver Framework (UMDF)](https://blog.th0m.as/misc/fingerprint-reversing/#usermode-driver-framework-umdf)
- [Debugging the Driver](https://blog.th0m.as/misc/fingerprint-reversing/#debugging-the-driver)
	- [The Protocol](https://blog.th0m.as/misc/fingerprint-reversing/#the-protocol)
	- [Creating a Wireshark Disector](https://blog.th0m.as/misc/fingerprint-reversing/#creating-a-wireshark-disector)
	- [Debugging Initialization, changing the PSK](https://blog.th0m.as/misc/fingerprint-reversing/#debugging-initialization-changing-the-psk)
	- [Some notes on reversing the TLS functions](https://blog.th0m.as/misc/fingerprint-reversing/#some-notes-on-reversing-the-tls-functions)
- [Figuring out the Image Format](https://blog.th0m.as/misc/fingerprint-reversing/#figuring-out-the-image-format)
- [Creating a Python Driver PoC](https://blog.th0m.as/misc/fingerprint-reversing/#creating-a-python-driver-poc)
- [Future](https://blog.th0m.as/misc/fingerprint-reversing/#future)
...
![](https://blog.th0m.as/processed_images/724395a146e8fa4f00.png)
...
![](https://blog.th0m.as/processed_images/704014c3cb1b532c00.png)
...
![](https://blog.th0m.as/processed_images/951a5e8a7a2213ce00.png)

Goodix Folder, containing logs. Logs contain lots of encrypted and base64 encoded data.

I then looked for processes which have one of these dll's loaded (for example by running `tasklist /M GoodixEngineAdapter.dll`). This turned out to be `svchost`. I also ran Procmon and filtered by goodix. This showed lots of reads and writes to the goodix logfiles by `svchost`.
...
![](https://blog.th0m.as/processed_images/fb4fb3d11886e8ef00.png)
...
![](https://blog.th0m.as/processed_images/1246b84dd952d38e00.png)

Usermode Driver Framework (UMDF). Microsoft Documentation, CC BY 4.0
...
Our Linux driver should be able to do the same! But it seems Goodix is really worried about an attacker sniffing packets, and wanted to prevent a passive observer to sniff the PSK. As such, the PSK is not transmitted in the clear, but encrypted with a whitebox.
...
![](https://blog.th0m.as/processed_images/ae1a613a6ff41d6600.png)
...
By writing our own PSK to the device, we are able to establish communication and receive an image. The decrypted image has substancially less entropy, so decryption is almost guaranteed to be correct. But a simple visualization still looks like noise with no fingerprint visible. We need to figure out how the image is encoded.
...
![](https://blog.th0m.as/processed_images/bba2233b868d52a200.png)
...
![](https://blog.th0m.as/processed_images/5972f4518a123f1e00.png)
...
It connects to the device, changes the PSK to all zero if needed, sets up a TLS connection and waits for the finger to be placed on the device and requests an image. As TLS is only specced to be used via TCP, all python libraries I found expect a socket to interact with. DTLS, which is intended to be wrapped in other protocols, is slightly different and thus incompatible.
