...
source: "https://imadrahmoune.com/pbr/"
author:
  - "[[imad]]"
published:
created: 2026-03-03
description: "Physically based rendering from first principles"
tags:
  - "clippings"
---
...
Where k e is a constant, q 1 and q 2 are the quantities of each charge, and r is the distance between them.
...
Then we plug R 0 in the approximation function for the reflectance:
...
| Macrogeometry | BRDF | Statistical average over a pixel, a larger roughness generates a wider cone |
...
Notable exceptions are **gold**, **copper**, **osmium** and **caesium**. Those metals absorb specific wavelengths which gives them their distinctive colors.
...
The second type of non-metallic materials we consider in our microfacet model are called **dielectrics**, these are materials that do not conduct electricity (insulators). They include **plastic, wood, glass, water, diamond, air...**

When a photon hits a dielectric material, it only gets absorbed if its energy matches the electron's energy in the material. So light either gets reflected, and the specular reflection depends on the roughness of the surface.
...
**L o** (p, ω o) = ∫ Ω **BRDF** (ω i, ω o, α) **L i** (p, ω i) (**n** ⋅ **ω i**) dω i

  
- L o (p, ω o) is the outgoing light at point p to the direction ω o
- L i (p, ω i) is the incoming light at point p from the direction ω i
- The **BRDF** (Bidirectional reflectance distribution function) is a function that tells use how much of the incoming light ω i is reflected to the outgoing direction ω o at point p, this function characterizes the surface of our material.
- The dot product is called the **cosine term**.

The rendering equation gives us the light reflected towards a direction L o at a point p by summing all the incoming lights L i at that point coming from direction ω i in the hemisphere Ω, weighted by the BRDF at that point and the cosine term.
...
BRDF(ω i, ω o, α) = k d diffuse + k s specular

It takes as input the incoming ω i and outgoing ω o light direction, and the roughness of the surface α. It equals the diffuse and the specular components weighted by their respective coefficients k d and k s.
...
The lambertian diffuse component is the diffuse color, called **albedo**, multiplied by the cosine factor. But since we already have the cosine factor in the rendering equation, the diffuse equation becomes: c π

f cook-torrance = D F G 4 (n⋅ω i) (n⋅ω o)

The Cook-Torrance specular component itself has three components: the normal distribution function D, the geometric function G and the Fresnel equation F.

## Normal Distribution Function

The normal distribution function is an approximation of the number of microfacets oriented in such a way that they will reflect light from the incoming direction ω i to the outgoing direction ω o.

The one we will use is the **Trowbridge-Reitz GGX** function:

D ggxtr (n, ω h, α) = α 2 π ((n⋅ω h) 2 (α 2 - 1) + 1) 2

ω h is the halfway vector between the incoming and outgoing directions, we calculate it like this:
...
G ggx (n, ω i, ω o, k) = G shadowing \* G masking

G ggx (n, ω i, ω o,k) = G(n, ω i, k) \* G(n, ω o, k)

G(n, ω, k) = n⋅ω n⋅ω \* (1 - k) + k
...
F schlick (ω i, F 0) = F 0 + (1 - F 0) \* (1 - ω i ⋅ω o) 5
