...
source: "https://mynameistrez.github.io/blog/creating-the-perfect-modding-language"
author:
  - "[[Trez’ blog]]"
published: 2024-02-29
created: 2025-08-30
description: "If you haven’t seen it yet, I recommend watching this blog post’s video called Creating grug: the perfect modding language first."
tags:
  - "clippings"
---
...
<video src="https://github.com/user-attachments/assets/31959bcf-e933-4080-bbb6-3c76fe8bfa39" width="100%" controls=""></video>
...
![](https://github.com/user-attachments/assets/2d67d359-8d13-4d38-92cf-8eb646a300aa)
...
Instead, the game should track which local allocations have been made at the start of every `on` function call, as that allows the game to automatically free all of those allocations immediately after the `on` function call. Similarly, the game should track which global allocations each entity has made, so they can all be freed once the entity is despawned.
...
This means that a game function like `list_add` will need to make a copy of the local `entity` it was passed, as the local `entity` gets freed at the end of the `on_tick` function. Copies are basically free if you have any way of reference counting the object. If it didn’t make a copy, the below `print_list` game function call would try to print a freed entity during the second `on_tick` call:
...
Instead of having the game functions `hash_map_string`, `hash_map_i32`, `hash_map_f32`, etc., the [grug mod loader for Minecraft](https://github.com/MyNameIsTrez/grug-mod-loader-for-minecraft) offers a generic `hash_map` game function that only stores `id` s. You store an `i32` in it by using the game function `box_i32`, which wraps an `i32` in an object, and returns its `id`. You unwrap the `i32` in it using `unbox_i32`. This was inspired by [Java’s autoboxing](https://docs.oracle.com/javase/tutorial/java/data/autoboxing.html).
...
<video src="https://github.com/user-attachments/assets/44b1920e-c0c8-489e-a92e-6f3d9530b9ab" width="100%" controls=""></video>
...
<video src="https://github.com/user-attachments/assets/a1769875-cc83-4e2b-a7aa-57a546b9ba64" width="100%" controls=""></video>
...
If you’re curious *how* grug catches runtime errors, [I wrote a post](https://mynameistrez.github.io/blog/how-grug-used-to-catch-runtime-errors) about the old implementation that used signal handlers. grug doesn’t use the slow signal handlers anymore, since it now for example just inserts machine code that checks whether the CPU overflow flag has been set, after every addition operation.
...
<video src="https://github.com/user-attachments/assets/4b7d6e3b-2be5-46a0-ab0c-b2caaf5532b7" width="100%" controls=""></video>
...
<video src="https://github.com/user-attachments/assets/df0a5332-0b35-4420-85c6-73b04fea0f83" width="100%" controls=""></video>
